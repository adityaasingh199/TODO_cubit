              ...
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Center(child: Text("All",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: flag !=1?Colors.black : Colors.white),)),
                  ),
                ),
                GestureDetector(
                  onTap: (){
                  ...
                  child: Container(
                    ...
                    child: Center(child: Text("Pending",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color:flag !=2?Colors.black : Colors.white,),)),
                  ),
                ),
                GestureDetector(
                  onTap: (){
                  ...
                  child: Container(
                    ...
                    child: Center(child: Text("Completed",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 15,color: flag !=3?Colors.black : Colors.white),)),
                  ),
                ),
              ],
            ),
            ...
            BlocBuilder<listCubit,listState>(builder: (_,state){
              return Container(
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (_,index){
                  return flag!=1 ?(flag!=2?(!state.mNotes[index].isCompleted ?Container() :ListTile(
                    onTap: (){
                      ...
                      Navigator.push(context, MaterialPageRoute(builder: (context)=>inputPage(index: state.mNotes[index].nID!, nTitle: state.mNotes[index].nTitle, nDesc: state.mNotes[index].nDesc)));
                    },
                    onLongPress:(){
                      //context.read<listCubit>().removeValue(index: index);
                    },
                    leading: Checkbox(value:state.mNotes[index].isCompleted , onChanged: (value){
                      context.read<listCubit>().updateIsComplete(Id: state.mNotes[index].nID!, isComplete: value ?? false);
                    }),
                    title: Text(state.mNotes[index].nTitle),
                    subtitle: Text(state.mNotes[index].nDesc),
                  )):state.mNotes[index].isCompleted ?Container() :ListTile(
                    onTap: (){
                      //Navigator.push(context, MaterialPageRoute(builder: (context)=>inputPage(index: index, nTitle: state.mData[index]["title"], nDesc: state.mData[index]["desc"])));
                      Navigator.push(context, MaterialPageRoute(builder: (context)=>inputPage(index: state.mNotes[index].nID!, nTitle: state.mNotes[index].nTitle, nDesc: state.mNotes[index].nDesc)));
                    },
